...
interface SpecRowOptions {
  highlight?: boolean;
  link?: string;
}

function specRow(label: string, value: string | null | undefined, options: SpecRowOptions = {}): string {
  if (value == null || value === '') return '';
  const { highlight = true, link } = options;
  const inner = link
...
function shell({ pre, kickerText, title, bodyHtml }: ShellOptions): string {
  return `<!doctype html>
<html lang="en"><head>
<meta charset="utf-8">
...
const firstName = (name: string = ''): string => esc(name.trim().split(/\s+/)[0] || 'there');

const refCode = (lead: Lead): string => {
  const source = (lead.email || '') + (lead.name || '');
  const hash = [...source].reduce((a, c) => (a * 31 + c.charCodeAt(0)) | 0, 7);
  return 'FJML-' + String(Math.abs(hash) % 100000).padStart(5, '0');
};
...
export function leadNotificationEmail(lead: Lead): EmailTemplate {
...
export function leadConfirmationEmail(lead: Lead): EmailTemplate {
